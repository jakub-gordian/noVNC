...
import GestureHandler from "./input/gesturehandler.ts";
import Cursor from "./util/cursor.ts";
import Websock from "./websock.ts";
import KeyTable from "./input/keysym.ts";
import XtScancode from "./input/xtscancodes.ts";
import { encodings } from "./encodings.ts";
import RSAAESAuthenticationState from "./ra2.ts";
import legacyCrypto from "./crypto/crypto.ts";

import RawDecoder from "./decoders/raw.ts";
import CopyRectDecoder from "./decoders/copyrect.ts";
import RREDecoder from "./decoders/rre.ts";
import HextileDecoder from "./decoders/hextile.ts";
import ZlibDecoder from './decoders/zlib.ts';
import TightDecoder from "./decoders/tight.ts";
import TightPNGDecoder from "./decoders/tightpng.ts";
...
RFB.messages = {
    ...
    extendedClipboardCaps(sock: Websock, actions: number[], formats: Record<string, number>): void {
        ...
        for (let i = 0; i < numericKeys.length; i++) {
            ...
            loopOffset += 4;
            data[3] = data[3]! | (1 << numericKeys[i]!); // Update our format flags
        }
